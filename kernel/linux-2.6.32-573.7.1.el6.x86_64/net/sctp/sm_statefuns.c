					...
					sctp_cmd_seq_t *commands)
{
	...
	err_chunk = NULL;
	if (!sctp_verify_init(ep, asoc, chunk->chunk_hdr->type,
					...
					sctp_cmd_seq_t *commands)
{
	...
	/* Restore associtation pointer for command interpeter to
	 * provide it with a valid context in case it needs to manipulate
	 * the queues */
