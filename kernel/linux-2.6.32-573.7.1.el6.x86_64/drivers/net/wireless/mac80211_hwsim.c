...
static struct genl_multicast_group hwsim_mcgrps[] = {
					   ...
					   struct genl_info *info)
{
	...
	if (info->snd_pid != wmediumd_portid)
		return -EINVAL;
...
static int hwsim_get_radio_nl(struct sk_buff *msg, struct genl_info *info)
{
	...
	list_for_each_entry(data, &hwsim_radios, list) {
		...
		res = mac80211_hwsim_get_radio(skb, data, info->snd_pid,
					       info->snd_seq, NULL, 0);
		if (res < 0) {
			nlmsg_free(skb);
			goto out_err;
					 ...
					 void *_notify)
{
	...
	remove_user_radios(notify->pid);

	if (notify->pid == wmediumd_portid) {
...
static int hwsim_init_netlink(void)
{
	...
	if (rc)
		goto failure;
	rc = genl_register_mc_group(&hwsim_genl_family, hwsim_mcgrps);
#endif
	if (rc)
		goto failure;
...
#if 1 /* in RHEL */
	genl_unregister_mc_group(&hwsim_genl_family, hwsim_mcgrps);
#endif
