...
static struct pci_device_id qlge_pci_tbl[] = {
...
/* Check for vlan tag in inbound path and strip it from skb if present */
static inline void ql_check_rx_vlan(struct ql_adapter *qdev,
					...
					u32 length)
{
	...
	ql_check_rx_vlan(qdev, skb, &vlan_id);
					...
					u32 length)
{
	...
	/* Update the MAC header length */
	ql_update_mac_hdr_len(qdev, ib_mac_rsp, skb->data, &hlen);
					...
					struct ib_mac_iocb_rsp *ib_mac_rsp)
{
	...
	if (ib_mac_rsp->flags4 & IB_MAC_IOCB_RSP_HV) {
		...
		!(ib_mac_rsp->flags2 & IB_MAC_IOCB_RSP_V)) {
		/* TCP and non-VLAN packet in a page chunk that's been
		 * checksummed, Tack it on to our GRO skb and let it go.
		 */
...
static int ql_adapter_initialize(struct ql_adapter *qdev)
{
	...
	/* Set the default queue behavior. */
