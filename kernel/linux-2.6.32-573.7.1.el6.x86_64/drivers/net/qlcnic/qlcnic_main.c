...
static void qlcnic_set_netdev_features(struct qlcnic_adapter *adapter,
				       struct qlcnic_esw_func_cfg *esw_cfg)
{
	struct net_device *netdev = adapter->netdev;
...
int qlcnic_set_real_num_queues(struct qlcnic_adapter *adapter,
			       u8 tx_queues, u8 rx_queues)
{
	...
	if (tx_queues)
		netif_set_real_num_tx_queues(netdev, tx_queues);
			  ...
			  struct net_device *netdev)
{
	...
	size = adapter->drv_tx_rings * sizeof(struct qlcnic_host_tx_ring);
