...
static const struct net_device_ops_ext nv_netdev_ops_ext = {
...
static int nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	if (id->driver_data & DEV_HAS_CHECKSUM) {
		np->txrxctl_bits |= NVREG_TXRXCTL_RXCHECK;
		netdev_extended(dev)->hw_features |= NETIF_F_IP_CSUM |
			NETIF_F_SG | NETIF_F_TSO | NETIF_F_RXCSUM;
	}
	...
	set_netdev_ops_ext(dev, &nv_netdev_ops_ext);
