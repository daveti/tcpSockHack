				     ...
				     const char *buf, const size_t count)
{
	...
	if (rport->reconnect_delay <= 0 && delay > 0 &&
		...
		queue_delayed_work(srp_long_wq, &rport->reconnect_work,
				   delay * HZ);
	} else if (delay <= 0) {
...
static void srp_reconnect_work(struct work_struct *work)
{
	...
	if (res != 0) {
		...
		if (delay > 0)
			queue_delayed_work(srp_long_wq,
...
static void __rport_fail_io_fast(struct srp_rport *rport)
{
	...
	if (srp_rport_set_state(rport, SRP_RPORT_FAIL_FAST))
		return;
	__scsi_target_unblock(rport->dev.parent, SDEV_TRANSPORT_OFFLINE);
...
int srp_reconnect_rport(struct srp_rport *rport)
{
	...
	scsi_target_block(&shost->shost_gendev);
	/*
	 * RHEL6 doesn't support the blkdev request_fn_active, and
	 * from looking at upstream, they don't truly support it
	 * there either as I can't find any place in the block layer
	 * that ever sets this item.  As a compromise, wait for
	 * 500 milliseconds which should allow anything that's
	 * going to complete to actually do so, then proceed.
	...
	if (res == 0) {
		...
		__scsi_target_unblock(&shost->shost_gendev, SDEV_RUNNING);
		/*
		 ...
		 */
		__rport_fail_io_fast(rport);
		__scsi_target_unblock(&shost->shost_gendev,
