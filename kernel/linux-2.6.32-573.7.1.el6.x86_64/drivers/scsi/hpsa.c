...
#define HPSA_DRIVER_VERSION "3.4.4-1-RH4"
...
static int hpsa_slave_alloc(struct scsi_device *sdev)
{
	...
	if (sd != NULL) {
		sdev->hostdata = sd;
		if (sd->queue_depth)
			scsi_adjust_queue_depth(sdev, scsi_get_tag_type(sdev), sd->queue_depth);
 ...
 */
static __devinit int hpsa_kdump_hard_reset_controller(struct pci_dev *pdev, u32 board_id)
{
	u64 cfg_offset;
	u32 cfg_base_addr;
...
static int __devinit hpsa_wait_for_mode_change_ack(struct ctlr_info *h)
{
	int i;
	u32 doorbell_value;
	unsigned long flags;
...
static __devinit int hpsa_init_reset_devices(struct pci_dev *pdev, u32 board_id)
{
	int rc, i;
	void __iomem *vaddr;
...
/* return -ENODEV or other reason on error, 0 on success */
static int __devinit hpsa_enter_performant_mode(struct ctlr_info *h, u32 trans_support)
{
	int i;
