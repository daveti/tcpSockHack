...
void perf_event_task_tick(void)
{
	...
	__get_cpu_var(perf_throttled_seq)++;
	throttled = __get_cpu_var(perf_throttled_count);
	__get_cpu_var(perf_throttled_count) = 0;
				   ...
				   struct pt_regs *regs)
{
	...
	seq = __get_cpu_var(perf_throttled_seq);
	if (seq != hwc->interrupts_seq) {
		...
		if (unlikely(throttle
			...
			__get_cpu_var(perf_throttled_count)++;
...
static void swevent_hlist_free(struct rcu_head *head)
{
	struct swevent_hlist *hlist;

	hlist = container_of(head, struct swevent_hlist, rcu_head);
	kfree(hlist);
}
...
static int tp_perf_event_init(struct perf_event *event)
{
	int err;
